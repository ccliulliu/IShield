#!/usr/bin/env python3
"""
场景三：记忆中毒与上下文污染 — 智能体攻击脚本（默认 500 条）

用法:
  python attacks/scenario3_memory_poisoning/attack_agent.py
  python attacks/scenario3_memory_poisoning/attack_agent.py --count 500 --save-cases
  python attacks/scenario3_memory_poisoning/attack_agent.py --use-file
"""
...
def run(backend: str, report_path: str, data: dict):
    ...
    print(f"[*] 场景三：{data['scenario_name']}")
    print(f"[*] 目标: {backend}")
    print(f"[*] 用例: 对抗={n_adv} 越狱={n_jb} 攻击链={n_chain} 合计={data.get('generated_count', n_adv+n_jb+n_chain)}")

    done = 0
    total_steps = n_adv
    for c in data.get("jailbreak_cases", []):
        total_steps += len(c.get("turns", [1]))
        if c.get("tool"):
            total_steps += 1
    total_steps += sum(len(c.get("steps", [])) for c in data.get("agent_attack_chains", []))
    ...
    for case in data.get("jailbreak_cases", []):
        ...
        if "turns" in case:
            for ti, turn in enumerate(case["turns"], 1):
                blocked, reason, _, ms = detect_attack(backend, turn)
                reporter.add(
                    AttackOutcome(
                        case_id=f"{case['id']}-t{ti}",
                        scenario=data["scenario_id"],
                        attack_type=f"越狱/{case['mode']}/轮{ti}",
                        payload_preview=turn[:80],
                        blocked=blocked,
                        channel="detect",
                        detail=reason,
                        elapsed_ms=round(ms, 1),
            ...
            text = case.get("text", "")
            blocked, reason, _, ms = detect_attack(backend, text)
            blocked_any = blocked
            reporter.add(
                AttackOutcome(
                    case_id=case["id"],
                    scenario=data["scenario_id"],
                    attack_type=f"越狱/{case['mode']}",
                    payload_preview=text[:80],
                    blocked=blocked,
                    channel="detect",
                    detail=reason,
            ...
            if not blocked and case.get("tool"):
                b2, r2, raw2, ms2 = simulate_tool(
                    backend, case["tool"], case["params"]
                )
                blocked_any = b2 or raw2.get("result") in ("blocked", "confirm")
                reporter.add(
                    AttackOutcome(
                        case_id=f"{case['id']}-tool",
                        scenario=data["scenario_id"],
                        attack_type=f"越狱/{case['mode']}/工具",
                        payload_preview=f"{case['tool']} | {case.get('params', '')[:50]}",
                        blocked=blocked_any,
                        channel="simulate",
                        detail=r2,
                        elapsed_ms=round(ms2, 1),
                    )
                )
                done += 1
        if done % 50 == 0:
            print(f"  进度 {done}/{total_steps}...", flush=True)
        if done <= 5 or done % 100 == 0:
            mark = "BLOCK" if blocked_any else "ESCAPE"
            print(f"  [{mark}] {case['id']} {case['mode']}")
    ...
    for chain in data.get("agent_attack_chains", []):
        ...
        for i, step in enumerate(chain["steps"], 1):
            if step["action"] == "detect":
                ...
                blocked, reason, _, ms = simulate_tool(
...
def main():
    p = argparse.ArgumentParser(description="场景三 记忆中毒 攻击脚本")
    ...
    p.add_argument(
        "--output",
        default=os.path.join(ROOT, "attacks", "reports", "scenario3_report.json"),
    )
    args = p.parse_args()
